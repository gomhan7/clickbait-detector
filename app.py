...
with col_btn2:
    if st.button("✨ 판별하기 ✨", key="predict_button", use_container_width=True, type="primary"):
        ...
        st.markdown("---")
        st.markdown(
        f"<p style='font-size:18px;'>📊 판별 결과: <strong>{prob*100:.2f}%</strong></p>",
        unsafe_allow_html=True
        )
        ...
        if predicted_label == 1: # 모델이 낚시성(1)으로 예측한 경우
            st.markdown(
            f"<p style='font-size:18px;'>🚨 이 뉴스는 **낚시성 뉴스**일 확률이 `{percent_clickbait}%` 입니다!: <strong>{prob*100:.2f}%</strong></p>",
            unsafe_allow_html=True
            )   
